---
format: revealjs
---


## Today's Agenda {background-image="Images/background-news_v2.png" .center}

```{r}
# background-size="1920px 1080px"
library(tidyverse)
library(readxl)
```

<br>

::: {.r-fit-text}
**IV. Paper 2 - Making a Convincing Argument**

- Get and give feedback on the outlines
:::

<br>

<br>

::: r-stack
Justin Leinaweaver (Fall 2024)
:::

::: notes
Prep for Class

1. Review Canvas submissions

2. Open news web pages for prompts to discuss
    - CNN, NYT, Washington Post, The Guardian, Irish Times, etc.

:::


## Fuse 101: Our Most Important Goal! {background-image="Images/background-news_v2.png" .center}

<br>

**Our primary goal is to ensure everyone in this room connects to the Drury experience in meaningful ways.**

- Finding and connecting to communities on campus

- Finding a major and a great advisor

- Navigating the Drury bureaucracy

- Planning your four years at Drury

- Scheduling

::: notes

**Life Check-in: Everybody doing ok?**

<br>

**Anybody run into anything we can help with?**

:::



## What's happening in the news? Any big stories to discuss? {background-image="Images/background-news_v2.png" .center}

::: notes
**What's happening in the news? Any big stories to discuss?**

<br>

*Open news web pages for prompts to discuss*

- CNN, NYT, Washington Post, The Guardian, Irish Times, etc.
:::



## Farrell's (2010) Writing Advice {background-image="Images/background-news_v2.png" .center}

::: {.r-fit-text}
1. Read the Requirements for the Assignment

2. Avoid Data Dumps

3. Cut to the Chase

4. Organize, Organize, Organize
    - Macro, meso and micro structures matter

5. Exercises in Style
:::

::: notes

Before we dive into the outlines, let's refresh your memories on the Farrell advice on writing a convincing argument.

- **Any questions on these five points?**

:::



## To be convincing an argument should be logical, clear, credible and critical {background-image="Images/background-news_v2.png" .center}

<br>

- **Logical**: Do the premises support the conclusion?

- **Clear**: Is the paper easy to read and understand?

- **Credible**: Is there enough high quality evidence (cited using APA)?

- **Critical**: Does the paper address weaknesses in the argument (e.g. logic, evidence, consider counter-arguments)?

::: notes

Don't forget our writing rubric!

- **Questions on this?**

:::


## Paper 2 Outline {background-image="Images/background-news_v2.png" .center}

**Intro**

- What is the question?
- Why do we care?
- Thesis statement

**Reason 1 (premise)**

**Reason 2 (premise)**

**Reason 3 (premise)**

**Counter-Argument**

::: notes

Pairs: Share and get feedback

- Help your partner think about the rubric and the clarity of the premises.

<br>

New pairs: Share and get feedback

<br>

Time to reflect: Think about the feedback and revise to strengthen your outline. 

<br>

SLIDE: Assignment for Monday
:::


## For Next Class {background-image="Images/background-blue_triangles2.png" .center}

<br>

```{r, fig.align = 'center'}
knitr::include_graphics("Images/13_3-Assign.png")
```

::: notes

**Questions on the assignment?**

- Don't forget that your counter-argument needs evidence too!

:::


